...
print("Executing script: video_cuts.py")

def get_video_files(configs):
    print(f"Getting video files from: {configs['source_video_files']}")
...
def get_audio_meta_files(configs):
    print(f"Getting audio meta files from: {configs['temp_audio_configs']}")
...
def process_audio_meta_files(audio_meta_files, video_files, configs, min_clip_duration):
    print(f"Processing audio meta files: {audio_meta_files}")
    for file_name in audio_meta_files:
        ...
        with open(audio_meta_path, "r") as json_file:
            ...
            new_json_data, total_duration = process_single_audio_meta(json_data, video_files, min_clip_duration)
            trailer_path = generate_trailer_json(file_name, new_json_data, configs)
            add_trailer_duration(trailer_path, total_duration)

def process_single_audio_meta(json_data, video_files, min_clip_duration):
    print(f"Processing single audio meta data...")
    new_json_data = []
    sum_duration_from_last = 0
    total_duration = 0  # Reset total_duration for each file
...
def generate_trailer_json(file_name, new_json_data, configs):
    print(f"Generating trailer JSON for file: {file_name}")
...
def add_trailer_duration(trailer_path, total_duration):
    print(f"Adding trailer duration to file: {trailer_path}")
    with open(trailer_path, "r+") as jsonFile:
        data = json.load(jsonFile)
        data["trailer_duration"] = total_duration
        jsonFile.seek(0)  # rewind
        json.dump(data, jsonFile, indent=4)
        jsonFile.truncate()

def generate_video_cuts(configs):
    print(f"Generating video cuts with configs: {configs}")
...
if __name__ == "__main__":
    ...
    print(f"Script started with configurations: {configs}")
